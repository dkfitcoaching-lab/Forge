...
export default function Navigation({ activeTab, onTabChange, onCoachOpen, hasUnread }) {
  ...
  return (
    <>
      ...
      <motion.button
        onClick={onCoachOpen}
        className="fixed z-40 flex items-center justify-center rounded-2xl shadow-2xl cursor-pointer"
        style={{
          bottom: 88, right: 16, width: 52, height: 52,
      ...
      >
        ...
        {hasUnread && (
          <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full" style={{
            ...
            boxShadow: `0 0 8px ${theme.danger}80`,
          }} />
        )}
      </motion.button>
            ...
            {tabs.map(({ key, label, Icon }) => {
              ...
              return (
                <motion.button
                  ...
                  className="flex flex-col items-center gap-0.5 px-6 py-1.5 relative cursor-pointer"
                  ...
                  <AnimatePresence>
                    {active && (
                      <motion.div
                        ...
                        style={{ background: theme.accent, width: 24, boxShadow: `0 0 10px ${theme.accent}60` }}
