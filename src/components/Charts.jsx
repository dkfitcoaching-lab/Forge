// ══════════════════════════════════════════════════════════════
// FORGE SVG CHART SYSTEM
// Pure SVG charts — no dependencies, infinite precision
// ══════════════════════════════════════════════════════════════

export function BarChart({ data, C, height = 120, label }) {
      ...
      <svg width="100%" height={height} viewBox={`0 0 100 ${height}`} preserveAspectRatio="none">
        <defs>
          <linearGradient id="barGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <stop offset="100%" stopColor={C.accent3} stopOpacity="0.4" />
          </linearGradient>
        </defs>
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              {d.value > 0 && (
                <text x={x + barWidth / 2} y={y - 3} textAnchor="middle" fontSize="4.5" fill={C.text3} fontFamily="var(--m)">
...
export function LineChart({ data, C, height = 100, label, unit = "" }) {
    ...
    <div>
      ...
      <svg width="100%" height={height} viewBox={`0 0 100 ${height}`} preserveAspectRatio="xMidYMid meet">
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((pct) => {
          ...
          return (
            <g key={pct}>
              <line x1="8" y1={y} x2="98" y2={y} stroke={`${C.accent}08`} strokeWidth="0.3" />
        ...
        {points.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r="1.8" fill={C.bg} stroke={C.accent} strokeWidth="0.8" />
  ...
  return (
    <div style={{ textAlign: "center" }}>
      <svg width={size} height={size} viewBox="0 0 80 80">
        <circle cx="40" cy="40" r={r} fill="none" stroke={`${C.accent}10`} strokeWidth="5" />
        <circle
          cx="40"
          cy="40"
          r={r}
  ...
  return (
    <div style={{ textAlign: "center" }}>
      <svg width={size} height={size} viewBox="0 0 52 52">
        <circle cx="26" cy="26" r={r} fill="none" stroke={`${color}15`} strokeWidth="4" />
        <circle
          cx="26"
          cy="26"
          r={r}
